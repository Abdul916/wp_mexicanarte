...
final class Kirki_Modules_Webfonts_Link {
	...
	public function create_link() {
		...
		//Begin remove custom uploaded font from Google font lists
		$uploaded_fonts = get_theme_mod('tg_custom_fonts');
		$uploaded_fonts_arr = array();
		
		if(!empty($uploaded_fonts) && is_array($uploaded_fonts))
		{
			foreach($uploaded_fonts as $uploaded_font)
			{
				if(isset($uploaded_font['font_name']) && !empty($uploaded_font['font_name']))
				{
					$uploaded_fonts_arr[] = $uploaded_font['font_name'];
				}
			}
			
			foreach($link_fonts as $key => $link_font)
			{
				if(in_array($link_font, $uploaded_fonts_arr))
				{
					unset($link_fonts[$key]);
				}
			}
		}
