...
<div <?php post_class(array('blog-posts-'.$settings['layout'])); ?>>
	...
	?>
	<div class="post-wrapper smoove <?php echo esc_attr($animation_class); ?>" data-move-y="450px" data-rotate-x="24deg" data-delay="<?php echo intval($queue*100); ?>" data-minwidth="<?php echo esc_attr($smoove_min_width); ?>" data-offset="-30%">
		...
		?>
		    <div class="post-featured-image static">
			    ...
			    <div class="post-featured-image-hover <?php if(!empty($tg_enable_lazy_loading)) { ?>lazy<?php } ?>">
			     	<?php 
				     	$blog_featured_img_url = get_the_post_thumbnail_url($post_ID, 'avante-blog'); 
				     	$blog_featured_img_data = wp_get_attachment_image_src(get_post_thumbnail_id($post_ID), "avante-blog" );
				     	...
				     	{
				     ?>
				     <img <?php echo avante_get_blank_img_attr(); ?> <?php echo esc_attr($return_attr['source']); ?>="<?php echo esc_url($blog_featured_img_url); ?>" class="<?php echo esc_attr($return_attr['class']); ?> smoove <?php echo esc_attr($animation_class); ?>" alt="<?php echo esc_attr($blog_featured_img_alt); ?>"/>
				     <?php
					     }
					?>
